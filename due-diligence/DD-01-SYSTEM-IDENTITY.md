...
Yggdrasil is not a supply chain platform in the sense that term is commonly used. It does not position itself as the authoritative record for cross-enterprise data. It does not aggregate data from multiple organizations into a central lake for analytics, benchmarking, or network-effect monetization. It does not sit "above" its tenants.
...
The system does not pursue data gravity as a business strategy. Each tenant's data lives in that tenant's PostgreSQL instance. The central broker stores events in transit, not at rest. A tenant that leaves the system takes its data with it; nothing structurally prevents this.
...
The system does not monetize tenant data. Anonymized aggregate data may be used for product improvement per the contract terms, but no data marketplace, benchmarking service, or network analytics product is built or planned. This is a constraint on revenue diversification, but it is also the thing that allows the system to credibly claim it is not a data intermediary.
