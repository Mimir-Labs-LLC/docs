...
The number-one cause of failed AI in the enterprise is not the model. It is the fact that the question *"what is a customer?"* has five different answers in five different systems, none of which agree, and the agent has been given access to all of them. Without one authoritative definition per business object — enforced, not aspirational — an agent reasoning about customers, orders, parts, or inventory is reasoning about a probabilistic blend of contradictory definitions, and its outputs are correspondingly probabilistic.
...
Agents need to know what just happened, not what was happening at 11 PM yesterday in last night's batch. Every system change that matters to an agent — a state transition, a policy update, a new approval, a flagged exception — needs to be observable in real time, on a typed addressable stream, in the same authorization envelope as the rest of the system.
...
The substrate-first answer is that policy is a signed document — authored by the humans who own it, with cited authority and attached evidence and named signers and effective dates — and the document is the runtime artifact. The runtime sits between every action and the database write, and refuses any transition the active policy does not permit. The agent cannot violate policy because the substrate will not let the violation be recorded.
...
The ranking is what the seven axes produce. Different axes — *shipping AI features today,* *production deployment scale,* *budget at-hand,* *analyst recognition* — produce different rankings, and those rankings matter for different reasons. The substrate ranking is the one that determines whether AI agents in your enterprise will be useful and safe rather than impressive and risky.
...
The board question — *what's our AI strategy?* — has a real answer. It just is not the answer the largest vendors are pricing to sell you today. The real answer is the architecture underneath the question. Until the architecture is right, no amount of AI feature surface above it will produce an outcome anyone is willing to put their name on.
